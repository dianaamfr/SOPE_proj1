...
Procuramos para além disso apresentar o tamanho correto mesmo quando são usadas flags repetidas e/ou em conjunto mas por odem diferente (por exemplo a conjugação das opções `-b` e `-B <size>`)
...
##### Análise do comportamento do comando `du` nos computadores dos elementos do grupo
...
Tendo em conta que não se detetou a razão pela qual os valores apresentados nesse computador (que é mais antigo que os dos restantes membros do grupo), optou-se por basear o cálculo dos valores no que é apresentado  pelo `du` nos computadores da maioria do grupo.
...
1. O valor em blocos é convertido em bytes `bytesSize = stat_buf->st_blocks * 512 `
2. No caso de estarmos perante um subdiretório ou ficheiro de um (sub)diretório é somado o valor anterior ao tamanho total do subdiretório pois a conversão do tamanho do seu subdiretório consoante o tamanho dos blocos a apresentar só pode ser feito no fim
...
- a leitura das `flags` e do `path`, que não deve ser efetuada pelos subprocessos( os subprocessos leem as flags enviadas no pipe, lendo apenas um descritor dos argumentos
- a forma como lida com a receção dos sinais, em particular o `SIGINT`, e por isso, os `signal handlers` que lhe estão associdados
Optamos por, ao invés de utilizar uma variável ambiente para guardar o 'pid' do primeiro processo
- ...

Por essa razão, optamos por, para evitar utilizar uma variável ambiente para guardar o `pid` do primeiro processo, utilizar uma estratégia baseada no bloquei de um sinal(`SIGUSR1`), enviado pelos subprocessos a ele próprio. 
Inicalmente utiliza-se a função `blockSIGUSR1` para bloquear o sinal em todos os processos:
...
Sempre que é criado um novo processo filho através da chamada `fork()` o filho envia o sinal `SIGUSR1` a ele próprio antes do `exec()`: `kill(getpid(),SIGUSR1)`.
Assim, ao iniciar um processo, como o sinal é sempre bloqueado, vai existir um sinal pendente `SIGUSR1` em todos os subprocessos, o que nos permite diferenciar o primeiro processo de todos, o único que não terá esse sinal pendente.

A verificação da existência de um sinal `SIGUSR1` pendente é realizada através da seguinte função, que, no caso de existir um sinal `SIGUSR1` pendente, o remove da lista de sinais pendentes:
